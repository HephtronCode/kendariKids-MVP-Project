...
export default function Sidebar() {
	...
	// Safely calculate the dashboard path inside the component.
	// If the user object doesn't exist yet, it will fall back to '/'
	const dashboardPath = user?.role ? `/${user.role}/dashboard` : "/";

	// Core, working navigation links
	const navLinks = [
		{
			to: dashboardPath,
			...
			<nav className="flex flex-col gap-2 flex-1">
				{navLinks.map((link) => (
					<NavLink
						key={link.text}
						to={link.to}
						// The `end` prop is crucial for the dashboard link to work correctly
